...
{% highlight java linenos %}
HEAPSORT(A):
  BUILD_MAX_HEAP(A)         // build max-heap
  for i = A.length downto 2
    swap(A, 1, i)           // restore max element to the right position
    MAX_HEAPIFY(A, 1)       // maintain max-heap property
  return A
{% endhighlight %}
